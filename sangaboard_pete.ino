 ...
 * 
 * Modified to use step / dir drivers for bipolar steppers. D. P. SIddons, Oct. 2024.
 */
#include "StepperF_alt_pete.h"   //Fergus's hacked stepper library
#include <assert.h>
#include <EEPROM.h>
#include <limits.h>
...
#endif
#ifdef ARDUINO_AVR_SANGABOARD
  #define SANGABOARDv3
  #define BOARD_STRING "Sangaboard v0.3"
#endif
#ifdef TWO_PIN_STEP_DIR
  #define CUSTOM_BOARD
  #define BOARD_STRING "Step / Dir driver v0.1"
#endif
#ifndef BOARD_STRING
  #define SANGABOARDv2
...
void setup() {
  ...
  while (! Serial )
    delay(1);
    Serial.println(BOARD_STRING);
  // get the stepper objects from the motor shield objects
  #if (defined(SANGABOARDv2)) //&& (defined(FOUR_PIN)))
    motors[0] = new Stepper(8, 13, 12, 11, 10);
    motors[1] = new Stepper(8, 9, 8, 7, 6);
    motors[2] = new Stepper(8, 5, 4, 3, 2);
  #elif (defined(SANGABOARDv3)&& (defined(FOUR_PIN)))
    motors[0] = new Stepper(8, 8, 9, 10, 11);
    motors[1] = new Stepper(8, 5, 13, 4, 12);
    motors[2] = new Stepper(8, 6, 7, A5, A4);
  #elif (defined(CUSTOM_BOARD)&& (defined(TWO_PIN_STEP_DIR)))
    motors[0] = new Stepper(8,2,3,8);
    motors[1] = new Stepper(8,4,5,9);
    motors[2] = new Stepper(8,6,7,10);
    Serial.print("Custom board step/dir\n");
  #elif (defined(CUSTOM_BOARD)&& (defined(TWO_PIN_UNI)))
    motors[0] = new Stepper(4,2,3);
    motors[1] = new Stepper(4,4,5);
    motors[2] = new Stepper(4,6,7); 
...
void stepMotor(int motor, long dx){
  // set direction
  if (dx<0) motors[motor]->direction=0;
  if (dx>0) motors[motor]->direction=1;
...
//    Serial.println("making the move");
    bool finished = false;
...
//          Serial.println(scaled_t);
//          Serial.println(((float)distance_moved[i] + 0.5) * step_delay[i]);
//          Serial.println(" ");
//          Serial.println(distance_moved[i]);
//          Serial.println(displacement[i]);

          if(scaled_t > ((float)distance_moved[i] + 0.5) * step_delay[i]){
            stepMotor(i, dir[i]);
//            Serial.println("move_axes");
//            Serial.println(i);
...
//    Serial.println("Got command: <"+command+">");
    ...
    if(command.startsWith("move_rel ") || command.startsWith("mr ")){ //relative move
